...
const WORD_LIMITS = { Twitter: 50, Instagram: 100, Facebook: 200 };
...
const PostList = ({ posts, onDelete, onUpdate }) => {
  ...
  const handleSaveClick = (id, platform) => {
    ...
    if (countWords(editContent) > WORD_LIMITS[platform]) return;
          ...
          <div className="post-body">
            {editingId === post.id ? (
              <>
                ...
                />
                <div className="word-count">Words: {countWords(editContent)} / {WORD_LIMITS[post.platform]}</div>
              </>
            ) : (
              <p>{post.content}</p>
            )}
